...
| # | Item | Priority | Notes |
|---|------|----------|-------|
| 1 | **Cursor changes through the floating sidebar** | P2 | Hovering the auto-hidden sidebar while it's revealed shows the page's cursor — a link underneath turns the pointer into a hand. WebKit sets the cursor from its own hit-testing and a SwiftUI layer on top doesn't stop it. Same root cause as the new-tab overlay tell, which got fixed structurally instead. Here the fix is an AppKit view owning `cursorUpdate` for the panel's area; I built one earlier and reverted it, because a real `NSView` in a SwiftUI stack sits above the drawn content and can swallow clicks. Needs care, not guesswork. |
